...
(function (qs) {
    ...
    var Parser = /** @class */ (function (_super) {
         ...
         * @breviary 摘要
         */
        function Parser(globalObj, tokens) {
...
(function (qs) {
    ...
    var Runtime = /** @class */ (function () {
        function Runtime(globalObject) {
            /**
             * 脚本类数据
             */
            this.codeClassPathMap = {};
            /**
             * 所有的脚本类
             */
            this.codeClassList = [];
        ...
        Runtime.prototype.regedit = function (code) {
            var lex = new qs.Lexer(code); //词法分析
            var parser = new qs.Parser(this.g, lex.getTokens()); //语法分析
            for (var key in parser.classPathMap) {
                this.codeClassPathMap[key] = parser.classPathMap[key];
            }
            this.codeClassList = this.codeClassList.concat(parser.codeClassList);
            for (var i = 0; i < parser.codeClassList.length; i++) {
                var codeClass = parser.codeClassList[i];
                this.parseClass(codeClass);
            }
        };
        Runtime.prototype.run = function (mainClassPath) {
            var pathList = mainClassPath.split(".");
            var objectClass = this.g;
            for (var i = 0; i < pathList.length; i++) {
                objectClass = objectClass[pathList[i]];
            }
            if (!objectClass) {
            ...
            if (codeCallFunction instanceof qs.CodeFunction) {
                ...
                if (!codeCallFunction.isNew) {
                    ...
                    if (codeCallFunction.member[".this"]) {
                        var canChangeThis = true;
                        if (codeCallFunction.member instanceof qs.CodeMember) {
                            if (codeCallFunction.member instanceof qs.CodeMember &&
                                codeCallFunction.member.parent &&
                                codeCallFunction.member.parent instanceof qs.CodeMember &&
                                codeCallFunction.member.parent.value == 'super') {
                                canChangeThis = false;
                            }
                        }
                        if (canChangeThis) {
        ...
        Runtime.prototype.getContextValue = function (returnData, key) {
            ...
            if (returnData) {
                ...
                if (this.codeClass.owner) {
                    return this.g[this.codeClass.owner.value][key] || this.g[key];
                }
                else {
                    return this.g[key];
                }
        ...
        Runtime.prototype.getVariable = function (member, returnData) {
            if (member.parent) {
                ...
                members.reverse();
                // var result = members.length > 0 && this.context[this.getMemberValue(members[0], returnData)] != null ? this.context : this.globalObj;
                ...
                var codeClass = this.codeClassPathMap;
                ...
                if (member.value == "this") {
                    ...
                    if (result === undefined) {
                        ...
                        result = result[value] || this.g[value];
                    }
                }
            }
        ...
        Runtime.prototype.setVariable = function (member, value, returnData) {
            if (member.parent) {
                ...
                // }
                // members.reverse();
                // // var result = members.length > 0 && returnData.context[members[0].value] !== undefined ? returnData.context : this.globalObj;
                ...
                // var parent = null;
                // var name = null;
                // if (result == this.globalObj) {//从模块中找一遍
                //     var names = this.getPathList(this.codeClass);
                //     names.pop();
                //     var result = this.globalObj;
                //     for (var i = 0; i < names.length; i++) {
                //         result = result[names[i]];
                //     }
                //     if (member.parent) {
                ...
                //     }
                //     if (result) {
                //         result = member instanceof CodeCallFunction ? this.resolveOperand(member, returnData) : result[this.getMemberValue(member, returnData)] || this.globalObj[this.getMemberValue(member, returnData)];
                //     }
                // }
                var oldMember = member;
                var members = [member];
                ...
                // this.globalObj[this.getMemberValue(member)] = value;
